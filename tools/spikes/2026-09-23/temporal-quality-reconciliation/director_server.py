...
from incident_projection import public_task_state  # noqa: E402
...
class Director:
    ...
    async def _start(self, run_id: str, package_digest: str, package: dict) -> None:
        client = await self.client()
    ...
    def perform(self, command: dict, task_id: str, context_id: str) -> dict:
        ...
        if "trial_fault_profile" in command:
            raise Rejected("unsupported Director command field")
...
class DirectorTaskStore(TaskStore):
    ...
    async def get(self, task_id, context=None):
        ...
        handle = client.get_workflow_handle(run_id)
        status = await handle.query(FactoryRun.status)
        state = public_task_state(status["phase"])
        payload = None
        if status["phase"] in {"accepted", "child-aborted", "child-expired", "child-incident"}:
            result = await handle.result()
            state = public_task_state(status["phase"], result["status"])
