...
OPS = {"assign", "join", "synthesize", "quality", "repair", "accept", "release",
       "abort", "start-child", "parent-result", "route-value"}
...
def validate_graph(name: str, document: dict) -> set[str]:
    ...
    for step in steps:
        ...
        if parsed[sid]:
            ...
            allowed = {
                ...
                "abort": {"revision"}, "start-child": {"child"},
            ...
            if (sid not in {"old_wait", "director_wait", "child_gate"} or not isinstance(opts.get("prompt"), str)
            ...
            if sid == "child_gate":
                if form.get("properties") != {"decision": {"type": "string", "enum": ["child_completed", "child_failed"]},
                                              "child_run_id": {"type": "string"}} or form.get("required") != ["decision", "child_run_id"]:
                    raise ValueError("child gate form must require pinned child ID")
    ...
    if any(parsed[sid] and parsed[sid][0] == "start-child" for sid in by_id):
        if "child_gate" not in by_id or not any(parsed[sid] and parsed[sid][0] == "parent-result" and "child_gate" in ancestors(sid) for sid in by_id):
            raise ValueError("parent lacks gated public child result")
    return children
