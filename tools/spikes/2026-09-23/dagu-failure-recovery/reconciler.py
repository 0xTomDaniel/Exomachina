    ...
    with db_connect() as db:
        db.executescript("""
            CREATE TABLE IF NOT EXISTS engine_retries (
              ...
              attempts INTEGER NOT NULL, last_at REAL NOT NULL, completed_at REAL,
              PRIMARY KEY(run_id,step_id));
        """)
        rows = [dict(row) for row in db.execute("SELECT run_id,dag_name,state FROM runs "
                                                "WHERE state NOT IN ('released','aborted','failed','child_failed')")]
    results: list[dict] = []
    for row in rows:
        try:
        ...
        with db_connect() as db:
            ...
            db.execute("INSERT OR REPLACE INTO engine_retries VALUES (?,?,?,?,NULL)",
        ...
        with db_connect() as db:
            db.execute("UPDATE engine_retries SET completed_at=? WHERE run_id=? AND step_id=? "
                       "AND attempts=?", (time.time(), row["run_id"], step, attempts))
