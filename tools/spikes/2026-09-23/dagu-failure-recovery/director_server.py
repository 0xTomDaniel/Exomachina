...
class DirectorTaskStore(TaskStore):
    ...
    async def get(self, task_id, context=None):
        ...
        try:
            ...
            return None
        if projection["bridge_state"] == "failed" or projection["product_state"] == "child_failed":
            state = "failed"
        elif projection["engine_status"] == "succeeded" and projection["product_state"] in {"released", "aborted"}:
            state = "completed"
