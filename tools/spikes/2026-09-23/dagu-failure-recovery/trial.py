...
def run() -> dict:
    ...
    try:
        ...
        until(lambda: (n := native(ROOT, failed_parent)) and n["status"] == "failed",
              "failed child propagated to native parent", 50)
        until(lambda: ops.get_run(failed_parent)["bridge_state"] == "failed",
              "failed child bridge final", 20)
        ...
        assert evidence["failed_child"]["parent"]["nodes"]["child_gate"]["status"] == "succeeded"
        assert evidence["failed_child"]["parent_row"]["bridge_state"] == "failed"
        assert evidence["failed_child"]["task"]["status"]["state"] == "failed"
        assert evidence["failed_child"]["child_row"]["acceptance_count"] == 0
        assert evidence["failed_child"]["child_row"]["release_count"] == 0
