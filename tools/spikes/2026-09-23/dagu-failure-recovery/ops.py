...
sys.path.insert(0, str(PRIOR / "arbitration" / "common"))
...
def db_connect() -> sqlite3.Connection:
    ...
    db.executescript("""
        CREATE TABLE IF NOT EXISTS runs (
          ...
          public_result_json TEXT, failure_json TEXT,
          bridge_state TEXT NOT NULL DEFAULT 'waiting');
...
def get_run(run_id: str | None = None) -> dict:
    ...
    if run["state"] == "failed" and os.environ.get("DAG_RUN_ID") == run_id:
        raise ValueError("failed product run is sealed; authorize a new run for repair")
    return run
...
def quality(revision: str) -> str:
    run = get_run()
    if revision == "r1" and (runtime() / "fault-child-before-terminal").exists():
        raise RuntimeError("injected child failure before Quality or terminal outcome")
...
def start_child(child: str) -> dict:
    parent = get_run()
    ...
    if status is None:
        try:
            ...
            if engine_status(child, child_id) is None:
                raise
    return {"child_run_id": child_id, "state": "started"}
...
def parent_result() -> dict:
    ...
    if value["state"] == "failed":
        ...
        native = engine_status(child["dag_name"], child["run_id"])
        if (child["state"] != "failed" or not child["failure_json"]
            or not native or native["statusLabel"] != "failed"
            or child["acceptance_count"] or child["release_count"] or child["receipt_json"]):
            raise ValueError("child failure evidence changed before parent terminal failure")
        raise RuntimeError("pinned child exhausted native recovery: " + child["run_id"])
...
def main() -> None:
    ...
    parser.add_argument("operation", choices=["assign", "join", "synthesize", "quality", "route-value",
                                             "repair", "accept", "release", "abort", "start-child", "parent-result",
    ...
    try:
        ...
        elif op == "start-child": value = start_child(args.child)
