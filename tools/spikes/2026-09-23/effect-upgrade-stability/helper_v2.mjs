...
async function callBridge(op, data) {
  fs.appendFileSync(path.join(root, 'v2-bridge-invocations.jsonl'),
    JSON.stringify({ op, run: data?.run ?? null, revision: data?.artifact?.revision ?? data?.revision ?? null }) + '\n');
...
const FactoryLive = FactoryRun.toLayer(({ id, digest, packageDigest, docJson, inputJson }) => Effect.gen(function* () {
  ...
  while (true) {
    ...
    if (kind === 'parallel') {
      ...
      const outcome = parsed(yield* stage(`v2:${at}:quality:${candidate.revision}`, () => callBridge('review', {
