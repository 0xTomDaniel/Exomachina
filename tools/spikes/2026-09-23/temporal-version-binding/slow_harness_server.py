...
class Harness:
    ...
    def pending(self, task_id):
        with self.connect() as db:
            row = db.execute("SELECT * FROM pending WHERE task_id=?", (task_id,)).fetchone()
    ...
    def perform(self, command, task_id, context_id):
        ...
        gate = self.state.parent / "hold-source-assignments"
        if self.role == "capability" and gate.exists():
            with self.connect() as db:
                db.execute("BEGIN IMMEDIATE")
                db.execute("INSERT OR IGNORE INTO pending VALUES (?,?,?,?,?)", (
                    task_id, command["action_id"], command["run_id"],
                    command["definition_digest"], context_id))
            deadline = time.monotonic() + 120
            while gate.exists() and time.monotonic() < deadline:
                time.sleep(.1)
            if gate.exists():
                raise Rejected("bounded source assignment hold timed out")
        newly_created = False
        with self.connect() as db:
            db.execute("BEGIN IMMEDIATE")
...
class LedgerTaskStore(TaskStore):
    ...
    async def get(self, task_id, context=None):
        ...
        if record is None:
            pending = self.harness.pending(task_id)
            if pending is not None:
                return Task(id=task_id, context_id=pending["context_id"],
                            status=TaskStatus(state="working"),
                            metadata={"action_id": pending["action_id"],
