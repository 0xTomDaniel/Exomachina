...
from binding import DEPLOYMENT, QUEUE, PublicationStore, verify_closure  # noqa: E402
...
class Director:
        ...
        with self.connect() as db:
            db.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS runs (
                    ...
                    manifest_digest TEXT NOT NULL, build_id TEXT NOT NULL);
    ...
    async def _start(self, run_id: str, package_digest: str, package: dict,
        ...
        try:
            ...
            if (status["package_digest"] != package_digest or
                    status["manifest_digest"] != publication["manifest_digest"]):
        ...
        try:
            await client.start_workflow(FactoryRun.run, input_value, id=run_id,
                task_queue=QUEUE, execution_timeout=timedelta(minutes=10),
    ...
    def perform(self, command: dict, task_id: str, context_id: str) -> dict:
        ...
        if "trial_fault_profile" in command:
            raise Rejected("trial fault profiles unavailable in version lane")
        ...
        with self.connect() as db:
            ...
            if op == "start":
                ...
                publications = PublicationStore(self.catalog)
                if existing:
                    publication = json.loads((self.catalog / f"publication-{existing['manifest_digest']}.json").read_text())
                    if publication["build_id"] != existing["build_id"]:
                        raise Rejected("run build binding changed")
                else:
                    publication = publications.active()
                    if publication["package_digest"] != package_digest:
                        raise Rejected("package is not active publication")
                db.execute("INSERT OR IGNORE INTO runs VALUES (?, ?, ?, ?)",
                           (run_id, package_digest, publication["manifest_digest"], publication["build_id"]))
        ...
        if op == "start":
            sync(self._start(run_id, package_digest, package, publication))
        elif op == "abort":
    ...
    async def get(self, task_id, context=None):
        ...
        if status["phase"] in {"accepted", "child-aborted", "child-expired"}:
            ...
            payload = {"status": result["status"], "run_id": run_id,
                       ...
                       "interpreter_build": status["interpreter_build"],
