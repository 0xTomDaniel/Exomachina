...
class Director:
    ...
    def trial_faults(self, run_id: str, profile: str | None) -> dict:
        ...
        if profile is None:
            return {}
        if profile == "quality_committed":
            return {"quality_barrier": {"marker": str(base / f"quality-committed-{suffix}"),
                                         "hold_seconds": 8}}
        if profile == "acceptance_timer":
            return {"acceptance_timer_seconds": 12}
        if profile == "held_assignment":
            return {"assignment_barrier": {"instance": "counter_evidence",
                "marker": str(base / f"assignment-held-{suffix}"),
                "resume": str(base / f"assignment-resume-{suffix}"),
                "timeout_seconds": 180}}
        raise Rejected("unapproved trial fault profile")
