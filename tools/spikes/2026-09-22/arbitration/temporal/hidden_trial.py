...
async def start_direct(client, run, package, director, faults=None):
...
async def main():
    ...
    try:
        ...
        # V4 requires_scope succeeds after one repair. A second run of exactly
        # this immutable package is intentionally checked for divergence; the
        # frozen interpreter has no approved per-run input affecting synthesis.
        v4 = definitions["requires_success"]
        first = await start_direct(client, "hidden-v4-one-repair", v4, direct_authority,
            {"delay_assignment": {"counter_scope": 1.25}})
        ...
        assert second_result["child"]["acceptance"]["revision"] == "r2"
