...
**Final verdict rule:** an attempt qualifies only when its saved structural status is `structural-pass` **and** the post-write attestation for those exact final evidence bytes has `g4_final.pass: true`. The in-run scan and positive control remain in the evidence as observations; they do not replace the final-byte attestation. `G-6` remains a separate suite gate. R1-d semantic reading remains a separate orchestrator judgment.

| Finding | Change | Regression/observation |
| --- | --- | --- |
| F1 | `sf_attest.py` runs after the scenario exits. It starts a fresh synthetic positive control, scans the home, evidence directory, model-home logs and candidate files, and records a path/SHA manifest that includes the final evidence JSON, route histories, markdown and supplied console. It normalizes relative and `/tmp` alias paths, checks manifest SHA stability and referenced file hashes, zero hits, scanner count, structural redaction and checker SHA. Its separate JSON includes packet provenance against `git show 2d609e3:prototype/temporal-factory/QUALIFICATION.md` as an observation, not a new pass condition. | `test_review3_attestation_rejects_changed_final_evidence_bytes`, `test_review3_export_selector_includes_nested_histories`, `test_review3_relative_evidence_manifest_resolves_exact_bytes`; attempt 12 final attestation passes. All 10 packet excerpts match; 10 required terminal newline trimming. |
| F2 | Removed the Director identity-table lookup and exact-token comparison. Scenario evidence fields under `token`, `actor`, `authorized_actor`, `owner_epoch`, `epoch`, `package`, `closure`, `bindings` and `input` are digested before export. The in-run and final verifier inspect raw JSON and recursively decoded base64 JSON, require the two-key redaction object at each sensitive key, and reject bearer/JWT-shaped strings. G-4 uses `leak_scan.sensitive_fields_redacted`, with exported-file coverage, in place of the removed token flags. | `test_review3_no_token_selection_or_credential_path`, `test_review3_decoded_sensitive_plaintext_fails`, existing history-redaction probe. The public, fixed `Bearer fixture-token` source constant in `harness_server.py` was not changed; it is not a credential value. |
| F3 | Saved the pre-launch home `lstat` result (`FileNotFoundError`), served factory Agent Card JSON body and canonical SHA-256, and only the broker status fields actually used: `signed_in`, `expired`, account hash and `expires_at`. SF-0 reads these artifacts. | `test_review3_sf0_artifacts_and_labels`; attempt 12 SF-0 passes. |
| F4 | Updated the requested behavior labels for R2-b/R3-b, R2-c, R2-d and R3-d/R3-e. Check predicates and pass/fail criteria are unchanged. | `test_review3_sf0_artifacts_and_labels`. |
| F5 | Saved packet source line-span comparison, match count and terminal-newline trimming in the attestation without adding it to `g4_final.terms` or the 24 scenario checks. The out-of-scope A2A and semantic coverage noted by the reviewer remains separate. | Attempt 12 packet provenance: 10/10 excerpts match. |

G-7 now requires the synthetic prerequisite's `--synthetic-attestation` path and SHA-256, a matching evidence SHA-256 inside that attestation, and `g4_final.pass: true`. A missing or failed attestation fails G-7. `test_review3_g7_requires_attestation` covers the missing case. A future live run must supply both `--synthetic-evidence` and `--synthetic-attestation`; no live run occurred here.

**Replay under corrected checker - not a new run:** the preserved live-2 JSON changes from 24/24 saved passes to **21/24** in memory. SF-0 fails because that old file lacks the new preflight/card/status artifacts. G-4 fails because it lacks structural redaction and final-byte coverage; G-7 fails because its synthetic prerequisite lacks a bound passing attestation. No saved evidence was edited or backfilled.

**Suites:** full Python suite under `/usr/bin/lockf -k /tmp/exo-qual-suite.lock` passed **174 tests** after the final checker/test edits. Node broker `npm test` under the same lock passed **19 tests**. The focused checker suite passed **23 tests**. `git diff --check` passed.

**Synthetic attempts:**

| Attempt | Preserved home and files | Structural replay | Final attestation | Disposition |
| --- | --- | --- | --- | --- |
| syn-11 | `/tmp/exo-sf-syn-11`, `evidence/single-factory/scripted-11.json`, `scripted-11-attestation.json`, all route files, `/tmp/exo-sf-syn-11-console.txt` | Saved 24/24 under the first checker; corrected replay 22/24, with G-4 and consequential G-7 failing because that in-run export selector counted only 3 files. | Re-attested with the corrected attester: `g4_final.pass: false` (`checker_bound: false`). Final redaction did inspect all 9 exports and found no violation; the historical checker SHA differs. | Does not qualify. All files and home retained. |
| syn-12 | `/tmp/exo-sf-syn-12`, `evidence/single-factory/scripted-12.json`, `scripted-12-attestation.json`, all route files, `/tmp/exo-sf-syn-12-console.txt` | Saved and corrected replay 24/24; in-run scan 2,673 files, 0 hits, 9 exported files checked, 32 decoded JSON payloads, 51 redaction objects, detected positive control. | `g4_final.pass: true`; final scan 2,235 files, 0 hits, 9 exported files checked, 32 decoded payloads, 61 redaction objects, detected fresh positive control, all required SHA and checker bindings pass. | Qualifies structurally under the final verdict rule; semantic R1-d remains pending. |

No third synthetic attempt was needed. Both homes and every generated evidence file remain preserved. The attestation excludes its own output from its input manifest to avoid a self-referential hash; it covers the final scenario evidence and the actual commit candidates available before that attestation write.

## Live attempt 1 diagnosis

24 September 2026. Read-only diagnostic: a `python -B` here-document loaded preserved `evidence/single-factory/codex-subscription-1.json`, called `check_evidence` helpers, and printed each Boolean term below. It did not contact a provider or broker, open a listener, or change evidence. The observations disprove the proposed single-cause explanation for G-2: old broker streams explain the missing session terms, while a separate checker term requires an in-window broker start even when the broker was already running. The table assigns **one classification per check** based on whether a checker correction is required; G-2 also needs the environment correction described below.

| Check | Failing term(s) on preserved live-1 | Classification | Quoted pre-registered text | Fix | Regression |
| --- | --- | --- | --- | --- | --- |
| R2-b | `_findings_on_claim` read `stimulus.claim_id = null` and found no exact quote in the finding's problem text. The served `r1` report has the exact planted claim as `C6`; the blocking finding names `C6`. Rejection, `decided_by:model`, and `_quality_bound` were true. | **(b) checker defect** | Brief: “At least one blocking finding identifies the planted claim: its `claim_id` equals the planted claim id, or its text quotes it.” A1: “only live `decided_by:"model"` verdicts can” support independence or detection. | Resolve the planted claim ID from the uniquely matching claim text in the served synthesis; compare the blocking finding's ID to that ID or its problem text to the exact planted text. | `test_live_attempt_1_corrected_predicates`: preserved C6 passes; wrong ID without a quote fails. |
| R3-b | The same `claim_id = null` term failed on `r1`, `r2`, and `r3`. Each served revision has the planted claim as `C7`, and each live blocking finding names `C7`. Three rejections, three bound reviews, `repair_count = max_repairs = 2`, and `repair:exhausted` were true. | **(b) checker defect** | Brief: “Quality (live, `decided_by:"model"`) rejects every revision, each with a blocking finding on its planted claim. The repair count reaches the pinned `max_repairs`, and the history shows `repair:exhausted`.” A1: “only live `decided_by:"model"` verdicts can” support independence or detection. | Use the same exact planted-claim binding for each revision. | `test_live_attempt_1_corrected_predicates`: preserved C7 findings pass; a wrong `r3` ID without a quote fails. |
| R3-d | `len(inspected) == 1` was false because it counted two accepted inspections: one on START and one on the follow-up. All follow-up-specific terms passed: one live turn with `inspect_run` then `decide_wait(abort, r3, current SHA)`, original Task/context, completed aborted Task, no artifact or release. | **(b) checker defect** | Brief: “The live Director turn calls `inspect_run`, then one accepted `decide_wait(abort, <current revision>, <current sha256>)`.” The preceding sentence binds this to the exact follow-up on the original Task/context. A1 makes no change to R3-d. | Count accepted inspections on that follow-up message; still require exactly one accepted wait decision across the Task and the exact follow-up turn sequence. | `test_live_attempt_1_corrected_predicates`: earlier START inspection is allowed; removal of the follow-up inspection fails. |
| G-2 | `broker_stable` was false because there were zero `start` events within the scenario window, although PID `59833` was unchanged before, during, and after and 35 `attach` events were recorded. Separately, 34 broker `stream` events had `session = null`: all 18 agent sessions were absent from streams, stream counts failed, and owner counts were authoring `4`, Director `30` versus recorded Director calls `12`. | **(b) checker defect** (with an environment term still failing) | Brief: “one broker PID across the scenario”; “each agent session appears in `broker-events.jsonl` `stream` events”; “Director and authoring calls are counted separately.” A1: “The scenario reads every agent's SQLite `tasks`/`model_calls` read-only (`mode=ro` URI) and preserves a copy in evidence.” | Accept zero in-window starts when a nonempty before PID equals all during/after samples and attaches exist; retain exact session and owner-count requirements. Next attempt must use the restarted integration broker that emits non-null session IDs, with 18 agent streams and 12 Director streams reconciled to their calls. | `test_live_attempt_1_corrected_predicates`: saved G-2 still fails; an in-memory reconstruction of the missing stream session fields passes; two in-window starts fail. This reconstruction is not a run. |
| G-5 | `len(ports_checked) >= 40` was false: the live block configured by the collector has exactly `13 + 5 + 1 + 20 = 39` ports. All 10 recorded processes and groups exited, listeners/errors were empty, service and runner stops were valid, and broker before/after PIDs matched. | **(b) checker defect** | Brief: “every service, harness and runner process the scenario started is stopped, and no listener remains in the block. The broker is left as found, with its PID before and after recorded.” A1 makes no change to G-5. | Require 39 scanned ports for live and 40 when the scripted mock port is present. | `test_live_attempt_1_corrected_predicates`: 39 live ports pass; removing a port fails. |

The diagnostic printed these remaining terms as true: R2-b's rejected/model/bound terms; R3-b's expected revision set, synthesis/review counts, rejection/binding for each revision, repair count and exhaustion; R3-d's exact follow-up text, original Task/context, one follow-up turn, ordered inspect/abort, live model, current revision/SHA, completed-aborted result, empty artifact/release; G-2's one nonempty sampled PID, expected Task IDs, store binding, provider/model/live fields, nonempty distinct agent sessions, positive author/Director call counts; and G-5's errors/listeners/started-process/exit/stop/PID terms. The exact failed terms are in the table.

**Replay under corrected checker - not a new run:** 23/24 pass. R2-b, R3-b, R3-d and G-5 change from fail to pass; G-2 remains fail because the saved broker streams lack sessions. The preserved evidence and saved verdicts were not edited.

Verification under `/usr/bin/lockf -k /tmp/exo-qual-suite.lock`: `tests.test_director_agent tests.test_harness_modes tests.test_sf_checks` passed, 31 tests. A repository-wide suite was not run: its broker tests operate a broker, and other tests bind ports outside lane D; this assignment expressly forbids both. No live provider or broker operation was performed.

## Fix: live G-7 binding

24 September 2026. The live G-7 check now binds a scripted prerequisite through its recorded evidence path, SHA-256, git commit, scripted provider, passing structural checks, complete route inventory, checker SHA-256, and interpreter build. It no longer compares the synthetic manifest digest with the live home's manifest digest: each fresh home gives its agents durable identities in its package bindings, so the manifests differ. The synthetic digest remains in the embedded prerequisite record for inspection. Regression tests cover a passing live-shaped record with a different manifest and failing records with a different interpreter build or checker SHA. The full Python suite under `/usr/bin/lockf -k /tmp/exo-qual-suite.lock` passed **166 tests**. The orchestrator will run the final-file synthetic prerequisite and live runs; this fix did not produce another scenario attempt or change committed evidence.
...
| F2 | SF-3 requires routes 1–3, unique caller/parent/child IDs, two exported histories each, one manifest/package/build, and the actual pinned enum. G-1 requires complete confirmed research, synthesis, Quality and release action counts with no incidents. G-7 uses the embedded synthetic evidence path, SHA and record, and checks its git commit, checker SHA, build and route inventory; the synthetic manifest is retained as evidence. | `test_f2_route_and_journal_completeness`, `test_f2_live_synthetic_evidence_binding`, `test_live_g7_*` |
