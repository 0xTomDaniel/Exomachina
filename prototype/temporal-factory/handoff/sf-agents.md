...
- Changed `test_template_materializes_and_validates_with_generated_bindings` to cover the **report profile**. It now uses `definitions/report-template.json`, generated report bindings, the committed `packets/exo-qualification-2026-09-23/packet.json`, and `authoring.materialize(..., evidence_packet=...)` before validating the package. The prior assertions for the legacy `outcome_mode` profile were removed from this test.
- Made `stimulus_log` unique on `(stimulus_id, task_id, revision)` with `INSERT OR IGNORE`. The planted canonical content is stored with its digest. `Ledger.finish` inserts or verifies the log and commits it in the same SQLite transaction as the completed Task artifact. Recovery from a previously committed log reuses that exact content without another model call. A restart test simulates process loss after the log commit and before Task completion; it verifies one row, the same claim ID and text, and `sha256_after` equal to the final artifact digest.
- Inlined `canonical()` and `sha256_text()` in `release_server.py` with the same JSON and UTF-8 hashing behavior as `fixture.py`, and removed its `src/` path insertion and import. `testbed.py` keeps the A1 launcher imports; legacy fixture services were not changed.
- Focused locked suites with the absolute qualification venv Python `-B`: `test_model_agent.py` passed 7 tests; `test_testbed.py` passed 3 tests. The locked full Python suite ran 138 tests and had exactly one error, the known lane D `test_harness_modes::test_publication_activates_without_starting_runner`: it calls `materialize(template, bindings)` without the now-required `evidence_packet`. The same sole error remained on the final full rerun. This lane did not edit lane D's test. `git diff --check` passed. No live broker was used.
