...
## Independent review follow-up (23 Sep 2026)

The credential path now requires `st_nlink === 1` at startup and on store reads, including checks on the opened descriptor. With a fixture base URL, startup also compares the fixture credential's `dev`/`ino` against the default install credential using `lstat` only; it never opens or reads the default credential. Hard-linked synthetic credentials fail as `config` before any fixture request.

Dead-PID lock recovery now has one recovery gate. Its winner rechecks the dead owner, atomically renames the stale directory to a unique tombstone, and removes only that tombstone. Other recoverers wait or observe the new live lock. A controlled test pauses one recoverer after it sees the stale owner, lets the other acquire the replacement lock, then resumes it and checks that the live lock inode survives and callbacks run serially.

Leak-scan follows file and directory symlinks, deduplicates targets by realpath, and avoids directory cycles. Only a path resolving to the canonical credential is excluded. Tests cover a symlinked token copy, a symlinked directory, a symlink to the canonical credential, and a hard link created after the scan's credential read. A hard link present at startup is refused by the credential policy.

The threat model accepts two same-user filesystem windows: swapping an ancestor directory after validation, and replacing a stale socket between its refusal check and unlink. This follow-up does not change those paths. Tests use synthetic credentials and loopback only, with the new recording fixture on port 46111. No real OpenAI contact or commit was made.

The recovery gate now records its owner PID under owner-only modes. A later process reclaims it only after re-verifying that recorded PID is dead. If the owner file is missing or malformed, recovery requires a 30-second-old gate, covering a crash between `mkdir` and the owner write. The winner moves the abandoned gate to its own unique tombstone and removes that tombstone. Tests use a killed child to prove prompt recovery, a live child with a backdated gate to prove preservation, and both fresh and old malformed-owner gates. The optional `withPidLock` hooks parameter is test-only: only the direct store-level interleaving test supplies hooks; CLI and socket paths reach `withPidLock` without that argument.

After this follow-up, `node --test broker/test/*.test.mjs` passed: 18 tests, 0 failures. No commit was made.
