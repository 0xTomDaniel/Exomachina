...
# Fix 3 (review 2: harness boundary)

- Factory A2A requests now reject any non-text part, including a DataPart carrying `start`, `abort`, `inspect`, or a mixed text/data message, with the fixed error `factory caller messages must contain text parts only`. Rejection happens in `HarnessExecutor` before `Director.invoke` or `Director.perform`. The text route still enters `DirectorTurn`. A second guard on `Director.invoke` rejects direct structured calls unless the instance config explicitly contains `"legacy_structured_commands": true`; `init_instance` defaults to false and stores the key only when a legacy fixture opts in. The report factory provision path does not opt in. Capability-role server behavior is unchanged.
- Caller-visible factory Task metadata no longer includes `authorized_input_actor` or `harness_incarnation`. Its accepted report artifact contains the report text and revision/SHA, acceptance, and release receipt without actor, epoch, or token fields. The internal `FactoryRun.status` query still exposes `owner_epoch` for harness fencing; this change does not alter that internal query. Lane D owns redaction of exported Temporal histories.
- Added `tests/test_factory_boundary.py`: DataPart start/abort and mixed-part rejection leave zero runs, commands, and Director tool calls; text enters a Director turn and invokes `start_research`; explicit legacy config permits the structured fixture path; public Task metadata and artifact contain no actor/epoch/token. The wire tests use FastAPI `TestClient`, a stub runner, and no live broker.
- Verification from `prototype/temporal-factory`: locked absolute-venv `python -B -m unittest discover -s tests` **151/151 passed** (`/tmp/exo-sf-boundary-python.log`); locked `npm --prefix broker test` **19/19 passed** (`/tmp/exo-sf-boundary-node.log`).
