...
BUNDLE = HERE.parent
RUNTIME = BUNDLE / "temporal"
TEMPORAL = RUNTIME / "temporal-server"
SQL_TOOL = RUNTIME / "temporal-sql-tool"
SCHEMA = RUNTIME / "temporal-1.32.0/schema/postgresql/v12"
PG_BIN = BUNDLE / "pg/bin"
CLI = BUNDLE / "bin/temporal"
PORTS = {"postgres": 31300, "frontend": 43002, "http": 43003, "matching": 43004,
         "history": 43005, "worker": 43006, "front_member": 31301,
         "match_member": 31302, "history_member": 31303, "worker_member": 31304,
         "pprof": 43011, "metrics": 43012}


def secret(name: str) -> str:
    return (STATE / "secrets" / name).read_text().strip()
...
def sql_command(database: str, *args: str) -> None:
    shell([str(SQL_TOOL), "--endpoint", "127.0.0.1", "--port", str(PORTS["postgres"]),
           "--user", "temporal", "--password", secret("temporal-password"),
           "--database", database, "--plugin", "postgres12", *args], timeout=120)


def start_postgres(first: bool) -> None:
    if first:
        shell([str(PG_BIN / "initdb"), "-D", str(STATE / "pgdata"), "--auth-local=scram-sha-256",
               "--auth-host=scram-sha-256", "--pwfile", str(STATE / "secrets/postgres-password"),
               "--no-instructions"], timeout=120)
    pg_command("-l", str(STATE / "postgres.log"), "-o",
               f"-p {PORTS['postgres']} -h 127.0.0.1 -k {STATE / 'pgsocket'} "
               f"-c shared_buffers={os.environ.get('EXO_PG_SHARED_BUFFERS', '16MB')} "
               f"-c max_connections={os.environ.get('EXO_PG_MAX_CONNECTIONS', '30')}", "start")
    if first:
        psql = str(PG_BIN / "psql")
        base = [psql, "-h", "127.0.0.1", "-p", str(PORTS["postgres"]), "-d", "postgres"]
        env = dict(os.environ, PGPASSWORD=secret("postgres-password"))
        shell(base + ["-c", "CREATE ROLE temporal LOGIN PASSWORD '" + secret("temporal-password") + "'"], env=env)
        for name in ("temporal", "temporal_visibility"):
            shell(base + ["-c", f"CREATE DATABASE {name} OWNER temporal"], env=env)
...
def render_config() -> Path:
    ...
    config = config.replace('password: "temporal"', 'password: "' + secret("temporal-password") + '"')
    config = config.replace('maxConns: 20', 'maxConns: ' + os.environ.get('EXO_TEMPORAL_MAX_CONNS', '8'))
    config = config.replace('maxIdleConns: 20', 'maxIdleConns: ' + os.environ.get('EXO_TEMPORAL_MAX_CONNS', '8'))
    config = config.replace('maxConns: 2', 'maxConns: 2')
